...
CREATE POLICY "Allow all operations for all auth users" ON "public"."order" TO "authenticated" USING (true) WITH CHECK (true);
...
CREATE POLICY "Enable update for auth users only" ON "public"."product" FOR UPDATE TO "authenticated" USING (true) WITH CHECK (true);



CREATE POLICY "Enable update for users based on email" ON "public"."category" FOR UPDATE TO "authenticated" USING ((( SELECT "users"."type"
   FROM "public"."users"
